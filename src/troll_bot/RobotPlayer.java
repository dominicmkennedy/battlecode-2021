...
public strictfp class RobotPlayer {
    ...
    static void runEnlightenmentCenter() throws GameActionException {
        ...
        while (true) {
            ...
            for (int id : toRemove) {
                if (muckIDs.contains(id)) {
                    muckIDs.remove(id);
                }
            }

            if (roundNum % 100 == 0 && currentFlag == 0) {
                scoutGoal = 2*scoutGoal;
            }
            ...
            if (currentInf > 100 || roundNum < 200) {
                if (roundNum <= 3) {
                    if (tryBuild(RobotType.SLANDERER, 41)) {
                        infInSlanders = infInSlanders + 41;
                    }
                } else if (roundNum < 35 && numScouts < scoutGoal) {
                    tryBuild(RobotType.MUCKRAKER, 1);
                } else if (roundNum < 45 && numDefenders < 4) {
                    tryBuild(RobotType.POLITICIAN, 15);
                } else if (currentInf >= 210 && nearestEnemyMuck >= 30 && infInSlanders < 600) {
                    if (tryBuild(RobotType.SLANDERER, Math.min(949, (int) (currentInf/10)))) {
                        infInSlanders = infInSlanders + Math.min(949, (int) (currentInf/10));  
                    }
                } else if (currentInf < 500 && income < 10 && nearestEnemyMuck > 15 && infInSlanders < 600) {
                    if (tryBuild(RobotType.SLANDERER, 41)) {
                        infInSlanders = infInSlanders + 41;
                    } else tryBuild(RobotType.SLANDERER, 21);
                        infInSlanders = infInSlanders + 21;
                } else if (numDefenders < 6) {
                    tryBuild(RobotType.POLITICIAN, 15);
                } else if (currentInf < 50) {
                    ;
                } else if (nearestEnemyEC != 0 && roundNum > 100) {
                    tryBuild(RobotType.POLITICIAN, 50);
                    if (rc.canSetFlag(nearestEnemyEC));
                        rc.setFlag(nearestEnemyEC);
                } else if (numScouts < scoutGoal) {
                    if (rc.getInfluence() > 5000 && Math.random() < 0.02) {
                        tryBuild(RobotType.MUCKRAKER, 1000);
                    } else {
                        tryBuild(RobotType.MUCKRAKER, 1);
                    }
                }
    ...
    static void runDefense() throws GameActionException {
        ...
        while (true) {
            ...
            // Move
            if (rc.getLocation().distanceSquaredTo(ecLoc) < 25 && tryMove(ecLoc.directionTo(rc.getLocation()))) {
                ;
            } else if (rc.getLocation().distanceSquaredTo(ecLoc) > 33 && tryMove(rc.getLocation().directionTo(ecLoc))) {
                ;
    ...
    static void runSlanderer() throws GameActionException {
        ...
        while (true) {
            ...
            if (!enemyLoc.equals(location)) {
                ...
                tryMove(dir);
            } else if ((location.x + location.y) % 2 == 0) {
                for (int i=0; i<4; i++) {
                    if (tryMove(directions[2*i])) break;
                }
            } else if (location.distanceSquaredTo(ecLoc) < 6) {
            ...
            } else if (location.distanceSquaredTo(ecLoc) > 13) {
    ...
    /**
     * Shortsightedly tries to move in a good direction to minimize cooldown
     *
     * @param dir Direction to destination
     * @return true if a move was performed
     * @throws GameActionException
     */
    static boolean greedyMove(Direction dir, MapLocation location) throws GameActionException {
        //System.out.println("I am trying to move " + dir + "; " + rc.isReady() + " " + rc.getCooldownTurns() + " " + rc.canMove(dir));
        MapLocation leftSpace = location.add(dir.rotateLeft());
        MapLocation rightSpace = location.add(dir.rotateRight());
        MapLocation centerSpace = location.add(dir);
        double leftPass = 0.0;
        double rightPass = 0.0;
        double centerPass = 0.0;
        
        if (rc.onTheMap(leftSpace)) {
            leftPass = rc.sensePassability(leftSpace);
            if (rc.isLocationOccupied(leftSpace)) {
                leftPass = 0.0;
            }
        }
        if (rc.onTheMap(rightSpace)) {
            rightPass = rc.sensePassability(rightSpace);
            if (rc.isLocationOccupied(rightSpace)) {
                rightPass = 0.0;
            }
        }
        if (rc.onTheMap(centerSpace)) {
            centerPass = rc.sensePassability(centerSpace);
            if (rc.isLocationOccupied(centerSpace)) {
                centerPass = 0.0;
            }
        }
        if (centerPass >= leftPass && centerPass >= rightPass) {
            if (rc.canMove(dir)) {
                rc.move(dir);
                return true;
            } else if (leftPass >= rightPass && rc.canMove(dir.rotateLeft())) {
        ...
        } else if (leftPass >= rightPass && leftPass >= centerPass) {
            if (rc.canMove(dir.rotateLeft())) {
                rc.move(dir.rotateLeft());
                return true;
            } else if (centerPass >= rightPass && rc.canMove(dir)) {
                rc.move(dir);
                return true;
            } else if (rc.canMove(dir.rotateRight())) {
                rc.move(dir.rotateRight());
        ...
        } else if (rightPass >= leftPass && rightPass >= centerPass) {
            if (rc.canMove(dir.rotateRight())) {
                rc.move(dir.rotateRight());
                return true;
            } else if (centerPass >= leftPass && rc.canMove(dir)) {
                rc.move(dir);
                return true;
    ...
    static boolean tryBuild(RobotType toBuild, int influence) throws GameActionException {
        for (int i = rc.getRoundNum(); i < rc.getRoundNum() + 8; i++) {
